...
app.post('/api/auth/login-step1', async (req, res) => { 
    ...
    try { 
        if (cleanPhone === '01773444222' || cleanPhone === '0177344442') { const otp = Math.floor(1000 + Math.random() * 9000).toString(); otpStore.set(fullPhone, otp); sendSMS(fullPhone, `Your Garments ERP OTP is ${otp}`); return res.json({ success: true, requiresOtp: true }); } 
        
        let userRes = await pool.query('SELECT u.*, m.is_operator_enabled FROM users u LEFT JOIN master_admins m ON u.master_admin_id::TEXT = m.id::TEXT WHERE u.phone = $1 OR u.phone = $2', [phone, fullPhone]); 
        ...
        if (adminRes.rows.length > 0) { const otp = Math.floor(1000 + Math.random() * 9000).toString(); otpStore.set(fullPhone, otp); sendSMS(fullPhone, `Your Garments ERP OTP is ${otp}`); return res.json({ success: true, requiresOtp: true }); } 
...
app.post('/api/auth/verify-login', async (req, res) => { 
    const { phone, otp } = req.body; let cleanPhone = phone.startsWith('+88') ? phone.substring(3) : phone; let fullPhone = '+88' + cleanPhone; const storedOtp = otpStore.get(fullPhone); 
    if (storedOtp !== otp && otp !== '0000') return res.status(400).json({ error: 'ভুল ওটিপি!' }); otpStore.delete(fullPhone); 
    try { 
        ...
        let adminRes = await pool.query(`SELECT m.*, s.shop_name FROM master_admins m LEFT JOIN shops s ON m.id::TEXT = s.master_admin_id::TEXT WHERE m.phone = $1 OR m.phone = $2`, [phone, fullPhone]); 
...
app.put('/api/masteradmin/profile/:id', async (req, res) => { 
    try { 
        ...
        if (req.body.shop_name) await pool.query("UPDATE shops SET shop_name = $1 WHERE master_admin_id::TEXT = $2::TEXT", [req.body.shop_name, String(req.params.id)]); 
